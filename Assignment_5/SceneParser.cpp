...
void SceneParser::parseFile() {
    ...
    while (getToken(token)) { 
        ...
        /*
        } else if (!strcmp(token, "OrthoCamera")) {
            parseOrthoCamera();
        */
...
/*
void SceneParser::parseOrthoCamera() {
    char token[MAX_PARSER_TOKEN_LENGTH];
    // read in the camera parameters
    getToken(token); assert (!strcmp(token, "{"));
    getToken(token); assert (!strcmp(token, "center"));
    ...
    getToken(token); assert (!strcmp(token, "}"));
    camera = new OrthoCamera(center,direction,up,size);
}
*/
...
Noise * SceneParser::parseNoise()
{
	...
	return new Noise(octaves, color[0],color[1],frequency,amplitude, type);
}
// ====================================================================
// ====================================================================
