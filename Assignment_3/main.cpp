...
namespace
{
  ...
  {
    if (argc != 4){
	cout << "USAGE: ./a3 <system> <INTEGRATOR> <STEP_SIZE>" << endl;
	exit(0);
    }

    // for in the 'stepSystem' function
    const char* h(argv[3]);
    step_size = atof(h);
    ...
    if(strcmp(argv[1], "simpleSystem")==0)
    {
        system = new SimpleSystem();		   
    }
    else if(strcmp(argv[1], "PendulumSystem")==0)
    {
        //cerr << endl << "Okay!" << endl;
        system = new simplePendulum();		   
    }
    else if(strcmp(argv[1], "chainSystem")==0)
    {
        system = new chainSystem();
    }
